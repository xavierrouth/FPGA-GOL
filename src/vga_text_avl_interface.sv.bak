...
`define CTRL_REG_IDX 600 //index of control register
...
vram vram_inst (.clk(CLK), .avl_in(AVL_WRITEDATA), 
				.avl_addr(AVL_ADDR[10:0]), .vga_addr(vram_addr), .avl_byteen(AVL_BYTE_EN), 
				.avl_rden(AVL_READ & AVL_CS & ~AVL_ADDR[11]), .avl_wren(AVL_WRITE & AVL_CS & ~AVL_ADDR[11]), .vga_rden(1'b1), 
...
vga_controller VGA(.Clk(CLK), .Reset(RESET), .hs(hs), .vs(vs), .pixel_clk(VGA_CLK), .blank(blank), .DrawX(drawx), .DrawY(drawy));
...
begin DRAW: 
	...
	// Raster Order
	// Row Major ORDER
	vram_addr = (drawx + ((drawy >> 4) * 640)) >> 4;
	// 1 Cycle of delay, but should just shift pixels so maybe not that big of a  deal
	vram_word = vram_data[(drawx[3:0] >> 3)]; // needs to select 0 or 1
	...
	bit_idx = drawx[2:0]; // Draw x mod 8
